...
//导航栏高度
#define NavAndStatusHeight ([MBProgressHUD isiPhoneX]?88:64)//(self.navigationController.navigationBar.frame.size.height+[[UIApplication sharedApplication] statusBarFrame].size.height)
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.removeFromSuperViewOnHide = YES;
    [view addSubview:hud];
    [hud setFrame:CGRectMake(0, NavAndStatusHeight, CGRectGetWidth(view.bounds), CGRectGetHeight(view.bounds) - NavAndStatusHeight*1)];
...
- (id)initWithView:(UIView *)view {
    ...
    return [self initWithFrame:CGRectMake(0, 100, CGRectGetWidth(view.bounds), CGRectGetHeight(view.bounds) - 200)];
}
...
- (void)setupViews {
    ...
    //MBBackgroundView *backgroundView =  [[MBBackgroundView alloc] initWithFrame:CGRectMake(0, 100, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - 200)];
...
- (void)updateConstraints {
    ...
    if (offset.y == 0) {
        offset.y = -NavAndStatusHeight/2.0;
    }
